...
var resizePizzas = function(size) {
  ...
  function changePizzaSizes(size) {
    ...
    var windowOffsetWidth = document.querySelector('#randomPizzas').offsetWidth;
    // to avoid forced reflow
    var cachedOffsetWidth = [];
    containers.forEach(function(item) {
        cachedOffsetWidth.push(item.offsetWidth);
    });
    for (var i = 0; i < containers.length; i++) {
      var container = containers[i];
      var dx = determineDx(cachedOffsetWidth[i], size, windowOffsetWidth);
      var newwidth = (cachedOffsetWidth[i] + dx) + 'px';
...
function updatePositions() {
  ...
  var adjustedScrollTop = document.body.scrollTop / 1250;
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin(adjustedScrollTop + (i % 5));
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var pizzasEl = document.querySelector('#movingPizzas1');
  for (var i = 0; i < 200; i++) {
